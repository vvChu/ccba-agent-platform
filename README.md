...
| **`ccba-legal-intel`** | Pipeline tự động hóa TVPL VIP, Sharded Legal Registry Compiler (`compile-registry`), đóng gói bộ chuẩn OKF Bundle v2.2 (`OKFBundlePackager`, `GoldStandardProcessor`), giải mã nhị phân công thức MathType MTEF (ADR 0040), kiểm định Visual Parity Gate 4, bóc tách phụ lục, AST diffing và hợp nhất Văn Bản Hợp Nhất (VBHN). | `pip install -e "packages/ccba-legal-intel"` |
