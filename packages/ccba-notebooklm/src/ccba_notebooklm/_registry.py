...
def get_notebook_id_for_workflow(workflow_name: str, catalog_path: Path) -> str:
    ...
    try:
        with open(catalog_path, encoding="utf-8") as f:
            ...
            items = catalog.get("skills", []) + catalog.get("workflows", [])
            for item in items:
                if isinstance(item, dict):
                    name = item.get("name", "")
                    clean_name = name.replace("ccba-", "")
                    clean_target = workflow_name.replace("ccba-", "")
                    if name == workflow_name or clean_name == clean_target:
                        bundle = item.get("bundle", "_core")
