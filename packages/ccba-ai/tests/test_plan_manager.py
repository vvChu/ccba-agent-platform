...
def test_phase_parsing_and_saving():
    with TemporaryDirectory() as tmpdir:
        ...
        custom_content = content.replace("priority: P2", "priority: P2\n# User custom comment here\ncustom_field: value")
        phase_file.write_text(custom_content, encoding="utf-8")
        
        # Reload, update status and save
        phase_to_update = Phase.from_file(phase_file, "01", "Init Project", "pending", "phase-01-test.md")
...
def test_plan_lifecycle_and_compatibility():
    with TemporaryDirectory() as tmpdir:
        ...
        # 1. Create plan
        res = create_plan(
            title="Refactor Platform",
            phases_list=["Phase A", "Phase B"],
