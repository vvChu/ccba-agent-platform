...
def rearrange_slides(
    ...
    """Create a new presentation with slides from template in specified sequence.

    Args:
        template_path: Path to template PPTX file.
        sequence: Either comma-separated string like '0,1,1,2' or list of ints.
        output_path: Path for output PPTX file.

    Returns:
        Path to generated output PPTX file.
    """
    if isinstance(sequence, str):
        seq_list = [int(x.strip()) for x in sequence.split(",") if x.strip()]
    else:
        seq_list = list(sequence)
    out_path = Path(output_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    rearrange_presentation(str(template_path), str(out_path), seq_list)
    return out_path
