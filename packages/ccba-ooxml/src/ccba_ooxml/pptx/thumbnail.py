...
def generate_thumbnails(
    ...
    """Generate thumbnail grid images from a presentation.

    Args:
        pptx_path: Path to PowerPoint presentation (.pptx)
        output_prefix: Prefix or path for generated images.
        cols: Number of columns in thumbnail grid (1-6).
        outline_placeholders: If True, draws red borders around text placeholders.
    ...
    prefix = str(output_prefix or in_path.with_suffix(""))
    output_path = Path(f"{prefix}.jpg")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if outline_placeholders:
            placeholder_regions, slide_dimensions = get_placeholder_regions(in_path)
        slide_images = convert_to_images(in_path, Path(temp_dir), CONVERSION_DPI)
        if not slide_images:
            return []
        grid_files = create_grids(
            slide_images,
        ...
        return [Path(gf) for gf in grid_files]
