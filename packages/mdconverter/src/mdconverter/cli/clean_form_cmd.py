    ...
    try:
        ...
        if recovered_title:
            try:
                ...
                try:
                    print(f"Recovered Title: {recovered_title.encode('utf-8', errors='ignore').decode('utf-8')}")
                except Exception:
                    pass
            console.print("[bold green]Success:[/bold green] Recovered and cleaned title successfully!")
        else:
            console.print("[yellow]No typical form placeholders or blank dotted lines detected in header. Skipping.[/yellow]")
