...
from ccba_legal.linter import lint_markdown_file, lint_bundle_links, lint_target_path
...
def test_lint_markdown_file_clean(tmp_path: Path) -> None:
    ...
    f = tmp_path / "doc.md"
    f.write_text("""# Điều 1. Phạm vi điều chỉnh
...
def test_lint_bundle_links(tmp_path: Path) -> None:
    ...
    main_doc.write_text("""# Văn bản chính
<a id="dieu-1"></a>
