...
def handle_structural_heading(
    ...
    if m_annex:
        ...
        if not a_title and i + 1 < len(blocks) and blocks[i + 1][0] == "p":
            ...
            if not re.match(r"^[0-9A-Z]+\.", ntxt2) and not ntxt2.startswith(
                ("Bảng", "Hình", "Phụ lục", "PHỤ LỤC")
            ):
                a_title = ntxt2
                i += 1
    ...
    if re.match(r"^(?:\*\*)?(?:CHÚ\s+THÍCH|Chú\s+thích)", text, re.IGNORECASE):
        ...
        prefix = (
            f"**CHÚ THÍCH {m_num.group(1)}:**" if m_num and m_num.group(1) else "**CHÚ THÍCH:**"
        )
        rendered_note = render_paragraph_with_runs(obj, rid_to_katex=ctx.rid_to_katex)
        rendered_note_clean = re.sub(
    ...
    if re.match(r"^(?:\t|\s)*(?:Đơn vị tính(?:\s+bằng)?|ĐƠN VỊ TÍNH)\s*(.+)$", text, re.IGNORECASE):
        unit_txt = re.sub(
            r"^(?:Đơn vị tính(?:\s+bằng)?|ĐƠN VỊ TÍNH)\s*", "", text.strip(), flags=re.IGNORECASE
        ).strip()
