...
**Tiêu chí đánh giá (đạt cả 3 → kích hoạt đề xuất):**
...
**Hành động khi đạt tiêu chí:**

Không chỉ ghi vào báo cáo — Agent phải hỏi người dùng ngay:

> *"Tôi phát hiện logic `[tên logic]` có thể đóng gói thành skill/workflow tái sử dụng.*
> *Bạn có muốn tôi ghi đề xuất này lên Hub ngay bây giờ không?*
> *Lệnh: `/propose-to-hub`"*

Nếu user đồng ý → Kích hoạt ngay workflow `/propose-to-hub` (đọc file:
`D:\GitHubProjects\ccba-agent-platform\.agent\workflows\propose-to-hub.md`)

Nếu user từ chối → Chỉ ghi vào mục "Đề xuất Skills mới" trong báo cáo tóm tắt để xem xét sau.
