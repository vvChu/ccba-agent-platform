...
| File | Mô tả |
|------|--------|
| `registry/legal_registry.yaml` | Danh mục VBPL đang theo dõi kèm metadata |
| `templates/comparison_table.md` | Template bảng so sánh VBPL cũ ↔ mới |
| `templates/impact_report.md` | Template báo cáo tác động thay đổi lên CCBA |
| `templates/notebooklm_prompts.md` | Prompt mẫu cho NotebookLM theo use case |

## How to Use

### 1. Cập nhật Registry VBPL

Đọc file `registry/legal_registry.yaml` để nắm danh mục hiện tại. Khi cần cập nhật:
...
1. Đọc template `templates/comparison_table.md`
...
1. Đọc template `templates/impact_report.md`
...
Đọc `templates/notebooklm_prompts.md` để lấy prompt mẫu cho các use case:
