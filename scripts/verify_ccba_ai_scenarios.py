...
def verify_scenario_1_routing() -> None:
    ...
    log_step(
        "Archetype Constants", ModelArchetype.OCR == "ocr-primary", "OCR constant matches contract"
    )
    log_step(
        "Archetype Reasoning",
        ModelArchetype.REASONING == "gemini-3.7-flash-high",
        "Reasoning matches contract",
    )
    log_step(
        "Choose Model OCR",
        choose_model("ocr") == "ocr-primary",
        "choose_model('ocr') -> ocr-primary",
    )
    log_step(
        "Choose Model Reasoning",
        choose_model("reasoning") == "gemini-3.7-flash-high",
        "choose_model('reasoning') -> gemini-3.7-flash-high",
    )
    log_step(
        "Choose Model Coding",
        choose_model("coding") == "gemini-3.7-flash",
        "choose_model('coding') -> gemini-3.7-flash",
    )
    log_step(
        "Choose Model Private",
        choose_model("private") == "qwen-local-primary",
        "choose_model('private') -> qwen-local-primary",
    )
    ...
    log_step(
        "Is Reasoning High",
        is_reasoning_model("gemini-3.7-flash-high") is True,
        "gemini-3.7-flash-high is reasoning",
    )
    log_step(
        "Is Reasoning Thinking",
        is_reasoning_model("claude-sonnet-4-6-thinking") is True,
        "claude-sonnet-4-6-thinking is reasoning",
    )
    log_step(
        "Is Standard Non-Reasoning",
        is_reasoning_model("gemini-3.7-flash") is False,
        "gemini-3.7-flash is not reasoning",
    )
...
def verify_scenario_2_sync_chat() -> None:
    ...
    log_step(
        "Sync Chat Response", len(reply) > 0, f"Got reply in {elapsed_ms:.1f}ms: '{reply.strip()}'"
    )
    ...
    log_step(
        "Sync Chat Multi Response",
        len(multi_reply) > 0,
        f"Got multi reply: '{multi_reply.strip()}'",
    )
...
def verify_scenario_3_telemetry_metadata() -> None:
    ...
    log_step(
        ...
        hasattr(result, "content") and hasattr(result, "usage") and hasattr(result, "latency_ms"),
        "ChatResult contains content, usage, latency_ms",
    )
...
def verify_scenario_4_circuit_breaker() -> None:
    ...
    # Record 2 failures
    dummy_err = RuntimeError("Simulated network connection failure")
    ...
    log_step(
        ...
        "Reached threshold -> tripped to OPEN",
    )
    ...
    try:
        ...
        log_step(
            "Fast-Fail Raised", True, f"Caught fast-fail error without network call: {e.to_json()}"
        )
    ...
    log_step(
        ...
        allowed is True and cb.state == CircuitState.HALF_OPEN,
        "After recovery timeout -> allow_request transitions to HALF_OPEN",
    )
    ...
    log_step(
        ...
        "Probe success resets CB to CLOSED",
    )
...
def verify_scenario_6_privacy_guard() -> None:
    ...
    log_step(
        "Privacy Violation Intercepted",
        blocked,
        "PrivacyGuardHook raised ValueError on sensitive key",
    )
    ...
    log_step(
        "AIClient Blocks Leak",
        client_blocked,
        "ai.chat() proactively rejected request containing API key",
    )
