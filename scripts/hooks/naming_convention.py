...
def main(event: str, payload: dict) -> int:
    ...
    if path_obj.stem.lower() in generic_names:
        print(f"[naming-convention] Warning: '{filename}' is a generic name. Use more descriptive file names.")
        return 0  # Warn only
    ...
    if ext == ".py":
        if not re.match(r"^[a-z_][a-z0-9_]*$", path_obj.stem):
            print(f"[naming-convention] Warning: Python file '{filename}' does not follow snake_case naming conventions.")

    # camelCase for JavaScript/TypeScript
    elif ext in [".js", ".ts", ".jsx", ".tsx"]:
        if not re.match(r"^[a-z][a-zA-Z0-9]*$", path_obj.stem):
            print(f"[naming-convention] Warning: JavaScript/TypeScript file '{filename}' does not follow camelCase naming conventions.")
