...
class GitRatchetTuner:
    ...
    def propose_mutation(self, current_content: str, iteration: int) -> str:
        ...
        # If the enhancement section header already exists, replace that section with the updated version
        section_header = enhancement.strip().split("\n")[0]
        if section_header in current_content:
            parts = current_content.split(section_header, 1)
            mutated = parts[0].rstrip() + enhancement
        elif enhancement.strip() in current_content:
            mutated = (
