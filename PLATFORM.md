...
│   ├── skills/                        ←   Reusable AI skills (<!-- SKILL_COUNT_START -->74<!-- SKILL_COUNT_END --> skills)
...
Hệ thống kỹ năng (<!-- SKILL_COUNT_START -->74<!-- SKILL_COUNT_END --> skills) được phân làm hai loại chính dựa trên cơ chế kích hoạt và tương tác:
...
*   **Quản lý Tài liệu & Quy trình:** `ccba-docs-manager` (qua `/ccba-docs-manager`), `ccba-handoff` (qua `/ccba-handoff`), `ccba-update-spoke` (qua `/ccba-update-spoke`).
*   **Soạn thảo & Đóng gói:** `ccba-copywriting` (qua `/ccba-copywriting`), `ccba-design` (thiết kế slides/brief), `ccba-init-spoke` (khởi tạo Spoke).
*   **Nghiệp vụ Xây dựng & Tư vấn:** `ccba-legal-document-tracker` (qua `/ccba-legal-document-tracker`), `ccba-completion-checklist` (quản lý HSHT), `ccba-seminar-builder` (qua `/ccba-seminar-builder`).
...
| `ccba-harness` | Testing harness, Two-Stage Granularity Decision Framework & GPI Calculator (ADR 0057), multi-scorer evaluation engine (SSOT Domain Archetypes `archetypes.py` with dedicated `bim_governance` and `bim_rase` subdomains, Domain Mock Simulation `simulation.py`, Orchestration Scorers, Token Budget Tracker, Real LLM Task Adapter, Auto-Tuner, DGX-ChatOps Interactive Skill Boost Action Buttons) | `pip install -e "packages/ccba-harness"` |
