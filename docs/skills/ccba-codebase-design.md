...
> **Mô tả ngắn**: Shared vocabulary for designing deep modules (locality, depth, leverage, seams) to improve testability and code quality. Reference skill.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-codebase-design
```
...
### Mục Đích Hoạt Động
Shared vocabulary for designing deep modules (locality, depth, leverage, seams) to improve testability and code quality. Reference skill.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Thiết kế kiến trúc sâu, đóng gói module và củng cố Deep Seams.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Phát hiện mã nguồn nông (Shallow Modules) / Cần tái cấu trúc**
...
Kỹ năng `ccba-codebase-design` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Phát hiện mã nguồn nông (Shallow Modules) / Cần tái cấu trúc ]
          │
          ▼
    >>> [ ccba-codebase-design ] <<<  (Thiết kế kiến trúc sâu, đóng gói module và củng cố Deep Seams.)
          │
          ▼
[ ccba-implement / ccba-to-spec ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Phát hiện mã nguồn nông (Shallow Modules) / Cần tái cấu trúc`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `ccba-implement / ccba-to-spec`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-codebase-design/SKILL.md --enforce-gpi
```
