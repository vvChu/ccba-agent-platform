...
> **Mô tả ngắn**: Master Skill quản lý và chuẩn hóa tài liệu Markdown từ Word/PDF qua Deep Seam ConversionPipeline.
...
| **Điểm Đánh Giá GPI (ADR-0057)** | `S=2.0 | K=2.0 | A=4.0 | P=1.0 (GPI: 15.50)` |
...
### Mục Đích Hoạt Động
Master Skill quản lý và chuẩn hóa tài liệu Markdown từ Word/PDF qua Deep Seam ConversionPipeline.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Chuyển đổi và chuẩn hóa tài liệu phức tạp sang Markdown chất lượng cao.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Tài liệu Word (.docx) hoặc PDF kỹ thuật**
...
Kỹ năng `ccba-markdown-document-processing` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Tài liệu Word (.docx) hoặc PDF kỹ thuật ]
          │
          ▼
    >>> [ ccba-markdown-document-processing ] <<<  (Chuyển đổi và chuẩn hóa tài liệu phức tạp sang Markdown chất lượng cao.)
          │
          ▼
[ Markdown chuẩn hóa / Knowledge Base ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Tài liệu Word (.docx) hoặc PDF kỹ thuật`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Markdown chuẩn hóa / Knowledge Base`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-markdown-document-processing/SKILL.md --enforce-gpi
```
