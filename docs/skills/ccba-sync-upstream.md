...
> **Mô tả ngắn**: Kiểm tra cập nhật và thẩm tra tính năng thượng nguồn (ADR-0057 Radar) kết hợp kích hoạt 1-Click Port qua /ccba-xia.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-sync-upstream
```
...
| **Điểm Đánh Giá GPI (ADR-0057)** | `S=4.0 | K=3.0 | A=3.0 | P=1.0 (GPI: 20.50)` |
...
### Mục Đích Hoạt Động
Kiểm tra cập nhật và thẩm tra tính năng thượng nguồn (ADR-0057 Radar) kết hợp kích hoạt 1-Click Port qua /ccba-xia.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Kiểm tra cập nhật và thẩm tra tính năng thượng nguồn (ADR-0057 Radar).
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Kho chứa thượng nguồn (ClaudeKit, Matt Pocock, v.v.)**
...
Kỹ năng `ccba-sync-upstream` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Kho chứa thượng nguồn (ClaudeKit, Matt Pocock, v.v.) ]
          │
          ▼
    >>> [ ccba-sync-upstream ] <<<  (Kiểm tra cập nhật và thẩm tra tính năng thượng nguồn (ADR-0057 Radar).)
          │
          ▼
[ ccba-xia / Khuyến nghị porting ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Kho chứa thượng nguồn (ClaudeKit, Matt Pocock, v.v.)`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `ccba-xia / Khuyến nghị porting`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-sync-upstream/SKILL.md --enforce-gpi
```
