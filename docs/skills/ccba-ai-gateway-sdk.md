...
> **Mô tả ngắn**: Kết nối AI Gateway trên Server Spark — Đa mô hình (local GPU + cloud), 1 endpoint. Bao gồm Python package ccba-ai.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-ai-gateway-sdk
```
...
### Mục Đích Hoạt Động
Kết nối AI Gateway trên Server Spark — Đa mô hình (local GPU + cloud), 1 endpoint. Bao gồm Python package ccba-ai.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Giao diện lập trình chuẩn kết nối AI Gateway đa mô hình trên Server Spark.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Nhu cầu gọi mô hình AI từ mã Python**
...
Kỹ năng `ccba-ai-gateway-sdk` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Nhu cầu gọi mô hình AI từ mã Python ]
          │
          ▼
    >>> [ ccba-ai-gateway-sdk ] <<<  (Giao diện lập trình chuẩn kết nối AI Gateway đa mô hình trên Server Spark.)
          │
          ▼
[ Kết quả suy luận từ AI Gateway ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Nhu cầu gọi mô hình AI từ mã Python`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Kết quả suy luận từ AI Gateway`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-ai-gateway-sdk/SKILL.md --enforce-gpi
```
