...
> **Mô tả ngắn**: Build, refine, and maintain the project's domain model, ubiquitous language, and record architectural decisions (ADRs).
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-domain-modeling
```
...
### Mục Đích Hoạt Động
Build, refine, and maintain the project's domain model, ubiquitous language, and record architectural decisions (ADRs).
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Xây dựng mô hình miền và ngôn ngữ chung (Ubiquitous Language).
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Bắt đầu bài toán nghiệp vụ phức tạp**
...
Kỹ năng `ccba-domain-modeling` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Bắt đầu bài toán nghiệp vụ phức tạp ]
          │
          ▼
    >>> [ ccba-domain-modeling ] <<<  (Xây dựng mô hình miền và ngôn ngữ chung (Ubiquitous Language).)
          │
          ▼
[ ccba-to-spec / Thiết kế Schema CSDL ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Bắt đầu bài toán nghiệp vụ phức tạp`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `ccba-to-spec / Thiết kế Schema CSDL`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-domain-modeling/SKILL.md --enforce-gpi
```
