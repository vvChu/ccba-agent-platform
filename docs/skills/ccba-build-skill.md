...
> **Mô tả ngắn**: Nghiên cứu tài liệu từ nhiều nguồn qua NotebookLM và tự động đóng gói sinh Skill mới đạt chuẩn CCBA.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-build-skill
```
...
| **Điểm Đánh Giá GPI (ADR-0057)** | `S=3.0 | K=2.0 | A=2.0 | P=1.0 (GPI: 14.00)` |
...
### Mục Đích Hoạt Động
Nghiên cứu tài liệu từ nhiều nguồn qua NotebookLM và tự động đóng gói sinh Skill mới đạt chuẩn CCBA.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Tạo khung kỹ năng mới tuân thủ Khung Quyết Định Hai Giai Đoạn (ADR-0057).
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Nhu cầu đóng gói quy trình mới thành kỹ năng**
...
Kỹ năng `ccba-build-skill` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Nhu cầu đóng gói quy trình mới thành kỹ năng ]
          │
          ▼
    >>> [ ccba-build-skill ] <<<  (Tạo khung kỹ năng mới tuân thủ Khung Quyết Định Hai Giai Đoạn (ADR-0057).)
          │
          ▼
[ ccba-eval-gate / SKILL.md chuẩn ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Nhu cầu đóng gói quy trình mới thành kỹ năng`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `ccba-eval-gate / SKILL.md chuẩn`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-build-skill/SKILL.md --enforce-gpi
```
