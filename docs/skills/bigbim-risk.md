...
> **Mô tả ngắn**: Phát hiện "Mâu thuẫn thông tin" (Information Conflict) phi hình học tại bước phối hợp thông tin V2 - Coordination, vượt ngoài giới hạn Clash Detection truyền thống.
...
```bash
python scripts/spoke/sync_spoke.py --skills bigbim-risk
```
...
### Mục Đích Hoạt Động
Phát hiện "Mâu thuẫn thông tin" (Information Conflict) phi hình học tại bước phối hợp thông tin V2 - Coordination, vượt ngoài giới hạn Clash Detection truyền thống.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Phát hiện xung đột thông tin phi hình học vượt ngoài va chạm vật lý.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **bigbim-rase / Mô hình phối hợp V2**
...
Kỹ năng `bigbim-risk` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ bigbim-rase / Mô hình phối hợp V2 ]
          │
          ▼
    >>> [ bigbim-risk ] <<<  (Phát hiện xung đột thông tin phi hình học vượt ngoài va chạm vật lý.)
          │
          ▼
[ ccba-ai-qc / Báo cáo xung đột thông tin ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `bigbim-rase / Mô hình phối hợp V2`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `ccba-ai-qc / Báo cáo xung đột thông tin`.
...
python scripts/validate_skills.py --file .agents/skills/bigbim-risk/SKILL.md --enforce-gpi
```
