...
> **Mô tả ngắn**: Tối ưu hóa PDF cho LLM: Phân đoạn (Segmenting), Chia nhỏ (Chunking) và Tiling cho AI Vision.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-ai-pdf-preprocessor
```
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Tối ưu hóa tài liệu PDF cho thị giác máy tính và trích xuất ngữ cảnh.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Tệp PDF bản vẽ hoặc hồ sơ dung lượng lớn**
...
Kỹ năng `ccba-ai-pdf-preprocessor` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Tệp PDF bản vẽ hoặc hồ sơ dung lượng lớn ]
          │
          ▼
    >>> [ ccba-ai-pdf-preprocessor ] <<<  (Tối ưu hóa tài liệu PDF cho thị giác máy tính và trích xuất ngữ cảnh.)
          │
          ▼
[ Ảnh tiled và văn bản phân đoạn cho LLM ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Tệp PDF bản vẽ hoặc hồ sơ dung lượng lớn`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Ảnh tiled và văn bản phân đoạn cho LLM`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-ai-pdf-preprocessor/SKILL.md --enforce-gpi
```
