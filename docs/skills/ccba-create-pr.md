...
> **Mô tả ngắn**: Kiểm tra chất lượng code (Shift-Left), Main Branch Guard, đẩy code và mở GitHub Pull Request kèm rào chắn Dual-Gate CI & Copilot Review
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-create-pr
```
...
### Mục Đích Hoạt Động
Kiểm tra chất lượng code (Shift-Left), Main Branch Guard, đẩy code và mở GitHub Pull Request kèm rào chắn Dual-Gate CI & Copilot Review
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Kiểm định Shift-Left, bảo vệ nhánh main, mở Pull Request và đồng hành CI.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Tính năng đã hoàn thành kiểm thử cục bộ (ccba-implement / ccba-tdd)**
...
Kỹ năng `ccba-create-pr` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Tính năng đã hoàn thành kiểm thử cục bộ (ccba-implement / ccba-tdd) ]
          │
          ▼
    >>> [ ccba-create-pr ] <<<  (Kiểm định Shift-Left, bảo vệ nhánh main, mở Pull Request và đồng hành CI.)
          │
          ▼
[ ccba-release-feature / GitHub Actions CI & Copilot Review ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Tính năng đã hoàn thành kiểm thử cục bộ (ccba-implement / ccba-tdd)`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `ccba-release-feature / GitHub Actions CI & Copilot Review`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-create-pr/SKILL.md --enforce-gpi
```
