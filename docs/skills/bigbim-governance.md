...
> **Mô tả ngắn**: Guardrails quản trị thông tin BIGBIM. Cưỡng chế tuân thủ Hiến pháp Sợi Chỉ Vàng, rào chắn Sợi Chỉ Đỏ và quy chuẩn định danh Unique ID từ giai đoạn A0.
...
### Mục Đích Hoạt Động
Guardrails quản trị thông tin BIGBIM. Cưỡng chế tuân thủ Hiến pháp Sợi Chỉ Vàng, rào chắn Sợi Chỉ Đỏ và quy chuẩn định danh Unique ID từ giai đoạn A0.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Cưỡng chế Hiến pháp Sợi Chỉ Vàng và rào chắn Sợi Chỉ Đỏ cho thông tin BIM.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Khởi động dự án BIM từ giai đoạn A0**
...
Kỹ năng `bigbim-governance` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Khởi động dự án BIM từ giai đoạn A0 ]
          │
          ▼
    >>> [ bigbim-governance ] <<<  (Cưỡng chế Hiến pháp Sợi Chỉ Vàng và rào chắn Sợi Chỉ Đỏ cho thông tin BIM.)
          │
          ▼
[ Phối hợp mô hình / Kiểm tra chất lượng ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Khởi động dự án BIM từ giai đoạn A0`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Phối hợp mô hình / Kiểm tra chất lượng`.
...
python scripts/validate_skills.py --file .agents/skills/bigbim-governance/SKILL.md --enforce-gpi
```
