...
> **Mô tả ngắn**: Phỏng vấn dồn dập người dùng về thiết kế (Stress-Test), đối chiếu quy chuẩn (Grill with Docs), hoặc hội tụ UI qua prototype trực quan.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-grilling
```
...
### Mục Đích Hoạt Động
Phỏng vấn dồn dập người dùng về thiết kế (Stress-Test), đối chiếu quy chuẩn (Grill with Docs), hoặc hội tụ UI qua prototype trực quan.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Phỏng vấn phản biện sâu để làm rõ các giả định và góc khuất kỹ thuật.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Ý tưởng sơ khai / Yêu cầu người dùng (Idea Phase)**
...
Kỹ năng `ccba-grilling` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Ý tưởng sơ khai / Yêu cầu người dùng (Idea Phase) ]
          │
          ▼
    >>> [ ccba-grilling ] <<<  (Phỏng vấn phản biện sâu để làm rõ các giả định và góc khuất kỹ thuật.)
          │
          ▼
[ ccba-to-spec / ccba-adr-lifecycle ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Ý tưởng sơ khai / Yêu cầu người dùng (Idea Phase)`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `ccba-to-spec / ccba-adr-lifecycle`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-grilling/SKILL.md --enforce-gpi
```
