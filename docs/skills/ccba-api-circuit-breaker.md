...
> **Mô tả ngắn**: Rate limiter + Circuit Breaker pattern cho LLM API calls trong batch pipelines. Tránh quota exhaustion, cascade failures, và infinite retry loops khi gọi AI Gateway hàng loạt.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-api-circuit-breaker
```
...
### Mục Đích Hoạt Động
Rate limiter + Circuit Breaker pattern cho LLM API calls trong batch pipelines. Tránh quota exhaustion, cascade failures, và infinite retry loops khi gọi AI Gateway hàng loạt.
...
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Tác vụ gọi LLM hoặc API bên ngoài quy mô lớn**
...
Kỹ năng `ccba-api-circuit-breaker` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Tác vụ gọi LLM hoặc API bên ngoài quy mô lớn ]
          │
          ▼
    >>> [ ccba-api-circuit-breaker ] <<<  (Kiểm soát tốc độ gọi API, ngắt mạch khi lỗi và tránh cạn kiệt hạn mức.)
          │
          ▼
[ Pipeline xử lý dữ liệu hàng loạt ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Tác vụ gọi LLM hoặc API bên ngoài quy mô lớn`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Pipeline xử lý dữ liệu hàng loạt`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-api-circuit-breaker/SKILL.md --enforce-gpi
```
