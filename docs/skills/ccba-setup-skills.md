...
> **Mô tả ngắn**: Thiết lập cấu hình dự án (Spoke/Hub) cho các công cụ kỹ thuật — cấu hình issue tracker, nhãn phân loại (triage), và bố cục tài liệu tri thức (Domain Docs). Chạy một lần trước khi sử dụng các kỹ năng phát triển phần mềm.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-setup-skills
```
...
| **Điểm Đánh Giá GPI (ADR-0057)** | `S=3.5 | K=2.0 | A=2.0 | P=1.0 (GPI: 15.25)` |
...
### Mục Đích Hoạt Động
Thiết lập cấu hình dự án (Spoke/Hub) cho các công cụ kỹ thuật — cấu hình issue tracker, nhãn phân loại (triage), và bố cục tài liệu tri thức (Domain Docs). Chạy một lần trước khi sử dụng các kỹ năng phát triển phần mềm.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Cài đặt hàng loạt kỹ năng tương ứng với Archetype của Spoke.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Khởi tạo hoặc cấu hình lại Spoke**
...
Kỹ năng `ccba-setup-skills` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Khởi tạo hoặc cấu hình lại Spoke ]
          │
          ▼
    >>> [ ccba-setup-skills ] <<<  (Cài đặt hàng loạt kỹ năng tương ứng với Archetype của Spoke.)
          │
          ▼
[ Toàn bộ quy trình tác nghiệp ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Khởi tạo hoặc cấu hình lại Spoke`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Toàn bộ quy trình tác nghiệp`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-setup-skills/SKILL.md --enforce-gpi
```
