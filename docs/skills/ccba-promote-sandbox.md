...
> **Mô tả ngắn**: Thăng cấp và bàn giao sản phẩm từ Spoke Cá Nhân sang Spoke Dự Án hoặc Hub (ADR 0046)
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-promote-sandbox
```
...
### Mục Đích Hoạt Động
Thăng cấp và bàn giao sản phẩm từ Spoke Cá Nhân sang Spoke Dự Án hoặc Hub (ADR 0046)
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Nâng cấp mã nguồn thử nghiệm thành thành phần chính thức của nền tảng.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Thử nghiệm thành công trong thư mục sandbox**
...
Kỹ năng `ccba-promote-sandbox` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Thử nghiệm thành công trong thư mục sandbox ]
          │
          ▼
    >>> [ ccba-promote-sandbox ] <<<  (Nâng cấp mã nguồn thử nghiệm thành thành phần chính thức của nền tảng.)
          │
          ▼
[ Mã nguồn chính thức của hệ thống ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Thử nghiệm thành công trong thư mục sandbox`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Mã nguồn chính thức của hệ thống`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-promote-sandbox/SKILL.md --enforce-gpi
```
