...
> **Mô tả ngắn**: Tạo và duy trì Danh Mục Hồ Sơ Hoàn Thành Công Trình theo VBPL hiện hành. Hỗ trợ xuất Markdown và Word (.docx).
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-completion-checklist
```
...
| **Điểm Đánh Giá GPI (ADR-0057)** | `S=3.0 | K=2.0 | A=4.0 | P=1.0 (GPI: 18.00)` |
...
### Mục Đích Hoạt Động
Tạo và duy trì Danh Mục Hồ Sơ Hoàn Thành Công Trình theo VBPL hiện hành. Hỗ trợ xuất Markdown và Word (.docx).
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Lập và duy trì Danh mục Hồ sơ Hoàn thành Công trình theo luật hiện hành.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Giai đoạn kết thúc thi công / Nghiệm thu bàn giao**
...
Kỹ năng `ccba-completion-checklist` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Giai đoạn kết thúc thi công / Nghiệm thu bàn giao ]
          │
          ▼
    >>> [ ccba-completion-checklist ] <<<  (Lập và duy trì Danh mục Hồ sơ Hoàn thành Công trình theo luật hiện hành.)
          │
          ▼
[ Hồ sơ hoàn thành công trình (Word/Markdown) ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Giai đoạn kết thúc thi công / Nghiệm thu bàn giao`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Hồ sơ hoàn thành công trình (Word/Markdown)`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-completion-checklist/SKILL.md --enforce-gpi
```
