...
> **Mô tả ngắn**: Guardrails to block or request explicit user permission before executing dangerous git operations (force push, hard reset, clean, etc.) via terminal.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-git-guardrails
```
...
### Mục Đích Hoạt Động
Guardrails to block or request explicit user permission before executing dangerous git operations (force push, hard reset, clean, etc.) via terminal.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Bảo vệ lịch sử Git, định dạng commit và ngăn ngừa xung đột nhánh.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Chuẩn bị commit hoặc tạo nhánh mới**
...
Kỹ năng `ccba-git-guardrails` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Chuẩn bị commit hoặc tạo nhánh mới ]
          │
          ▼
    >>> [ ccba-git-guardrails ] <<<  (Bảo vệ lịch sử Git, định dạng commit và ngăn ngừa xung đột nhánh.)
          │
          ▼
[ Git push / Pull Request ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Chuẩn bị commit hoặc tạo nhánh mới`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Git push / Pull Request`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-git-guardrails/SKILL.md --enforce-gpi
```
