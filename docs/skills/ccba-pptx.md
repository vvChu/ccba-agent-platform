...
> **Mô tả ngắn**: Công cụ tạo và chỉnh sửa file trình chiếu PowerPoint (.pptx) nâng cao bằng HTML conversion hoặc OOXML.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-pptx
```
...
### Mục Đích Hoạt Động
Công cụ tạo và chỉnh sửa file trình chiếu PowerPoint (.pptx) nâng cao bằng HTML conversion hoặc OOXML.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Tạo slide thuyết trình kỹ thuật chuyên nghiệp, tự động căn chỉnh bố cục.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Nội dung thuyết trình / Đề cương báo cáo**
...
Kỹ năng `ccba-pptx` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Nội dung thuyết trình / Đề cương báo cáo ]
          │
          ▼
    >>> [ ccba-pptx ] <<<  (Tạo slide thuyết trình kỹ thuật chuyên nghiệp, tự động căn chỉnh bố cục.)
          │
          ▼
[ File trình chiếu PowerPoint (.pptx) ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Nội dung thuyết trình / Đề cương báo cáo`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `File trình chiếu PowerPoint (.pptx)`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-pptx/SKILL.md --enforce-gpi
```
