...
> **Mô tả ngắn**: Quản trị hạ tầng SharePoint Online & M365 dạng mã nguồn (Infrastructure-as-Code). Hướng dẫn thiết kế JSON schema, kiểm định Lookups/Taxonomy và triển khai bằng PnP PowerShell.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-sharepoint-iac
```
...
| **Điểm Đánh Giá GPI (ADR-0057)** | `S=3.0 | K=2.0 | A=1.0 | P=1.0 (GPI: 12.00)` |
...
### Mục Đích Hoạt Động
Quản trị hạ tầng SharePoint Online & M365 dạng mã nguồn (Infrastructure-as-Code). Hướng dẫn thiết kế JSON schema, kiểm định Lookups/Taxonomy và triển khai bằng PnP PowerShell.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Quản lý hạ tầng SharePoint dưới dạng mã và đồng bộ hóa thư mục dự án.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Cấu trúc lưu trữ tài liệu dự án**
...
Kỹ năng `ccba-sharepoint-iac` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Cấu trúc lưu trữ tài liệu dự án ]
          │
          ▼
    >>> [ ccba-sharepoint-iac ] <<<  (Quản lý hạ tầng SharePoint dưới dạng mã và đồng bộ hóa thư mục dự án.)
          │
          ▼
[ Thư viện tài liệu SharePoint được đồng bộ ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Cấu trúc lưu trữ tài liệu dự án`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Thư viện tài liệu SharePoint được đồng bộ`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-sharepoint-iac/SKILL.md --enforce-gpi
```
