...
> **Mô tả ngắn**: Tự động hóa viết bảng thực thể (En) và phân loại vật tư (PM) theo Uniclass 200, tích hợp chuẩn ISO (22274, 21511, 12006-2), ISO 19650 Room Naming & IFC Alignment.
...
```bash
python scripts/spoke/sync_spoke.py --skills bigbim-classification
```
...
### Mục Đích Hoạt Động
Tự động hóa viết bảng thực thể (En) và phân loại vật tư (PM) theo Uniclass 200, tích hợp chuẩn ISO (22274, 21511, 12006-2), ISO 19650 Room Naming & IFC Alignment.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Tự động phân loại vật tư Uniclass và chuẩn hóa định danh thực thể ISO.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Mô hình BIM sơ bộ / Hồ sơ thiết kế**
...
Kỹ năng `bigbim-classification` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Mô hình BIM sơ bộ / Hồ sơ thiết kế ]
          │
          ▼
    >>> [ bigbim-classification ] <<<  (Tự động phân loại vật tư Uniclass và chuẩn hóa định danh thực thể ISO.)
          │
          ▼
[ bigbim-rase / bigbim-risk ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Mô hình BIM sơ bộ / Hồ sơ thiết kế`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `bigbim-rase / bigbim-risk`.
...
python scripts/validate_skills.py --file .agents/skills/bigbim-classification/SKILL.md --enforce-gpi
```
