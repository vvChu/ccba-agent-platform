...
> **Mô tả ngắn**: Tư vấn & giải đáp pháp lý xây dựng: Phỏng vấn thích ứng làm rõ ngữ cảnh và xuất Phiếu Ý kiến Pháp lý (Legal Opinion) chuẩn mực trích dẫn OKF v2.4.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-legal-advisor
```
...
### Mục Đích Hoạt Động
Tư vấn & giải đáp pháp lý xây dựng: Phỏng vấn thích ứng làm rõ ngữ cảnh và xuất Phiếu Ý kiến Pháp lý (Legal Opinion) chuẩn mực trích dẫn OKF v2.4.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Phỏng vấn làm rõ ngữ cảnh và tư vấn pháp lý xây dựng trích dẫn OKF v2.4.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Tình huống pháp lý xây dựng cần làm rõ**
...
Kỹ năng `ccba-legal-advisor` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Tình huống pháp lý xây dựng cần làm rõ ]
          │
          ▼
    >>> [ ccba-legal-advisor ] <<<  (Phỏng vấn làm rõ ngữ cảnh và tư vấn pháp lý xây dựng trích dẫn OKF v2.4.)
          │
          ▼
[ Phiếu Ý kiến Pháp lý (Legal Opinion) ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Tình huống pháp lý xây dựng cần làm rõ`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Phiếu Ý kiến Pháp lý (Legal Opinion)`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-legal-advisor/SKILL.md --enforce-gpi
```
