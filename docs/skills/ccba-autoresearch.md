...
> **Mô tả ngắn**: Khởi chạy vòng lặp tối ưu hóa kỹ năng AI tự động qua đêm (Git-Ratchet Auto-Tuner) lấy cảm hứng từ karpathy/autoresearch.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-autoresearch
```
...
### Mục Đích Hoạt Động
Khởi chạy vòng lặp tối ưu hóa kỹ năng AI tự động qua đêm (Git-Ratchet Auto-Tuner) lấy cảm hứng từ karpathy/autoresearch.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Quy trình nghiên cứu 2 vòng: Code-First Research và Self-Adversarial Review.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Chủ đề nghiên cứu khoa học / Kỹ thuật mới**
...
Kỹ năng `ccba-autoresearch` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Chủ đề nghiên cứu khoa học / Kỹ thuật mới ]
          │
          ▼
    >>> [ ccba-autoresearch ] <<<  (Quy trình nghiên cứu 2 vòng: Code-First Research và Self-Adversarial Review.)
          │
          ▼
[ Báo cáo nghiên cứu đa chiều ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Chủ đề nghiên cứu khoa học / Kỹ thuật mới`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Báo cáo nghiên cứu đa chiều`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-autoresearch/SKILL.md --enforce-gpi
```
