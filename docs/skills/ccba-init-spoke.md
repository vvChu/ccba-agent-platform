...
> **Mô tả ngắn**: Khởi tạo một dự án (Spoke) tuân thủ kiến trúc CCBA Agent Platform
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-init-spoke
```
...
### Mục Đích Hoạt Động
Khởi tạo một dự án (Spoke) tuân thủ kiến trúc CCBA Agent Platform
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Khởi tạo cấu trúc phân vùng Spoke chuẩn mực theo Archetype.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Thành lập dự án mới từ Hub**
...
Kỹ năng `ccba-init-spoke` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Thành lập dự án mới từ Hub ]
          │
          ▼
    >>> [ ccba-init-spoke ] <<<  (Khởi tạo cấu trúc phân vùng Spoke chuẩn mực theo Archetype.)
          │
          ▼
[ ccba-setup-skills / ccba-update-spoke ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Thành lập dự án mới từ Hub`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `ccba-setup-skills / ccba-update-spoke`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-init-spoke/SKILL.md --enforce-gpi
```
