...
> **Mô tả ngắn**: Kỹ năng tự động cào và đóng gói văn bản pháp luật VIP TVPL qua Deep Seam TVPLCrawler (tự động CookieVault & Mutex).
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-tvpl-vip-crawler
```
...
### Mục Đích Hoạt Động
Kỹ năng tự động cào và đóng gói văn bản pháp luật VIP TVPL qua Deep Seam TVPLCrawler (tự động CookieVault & Mutex).
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Thu thập nội dung số hóa bản quyền từ hệ thống TVPL VIP.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Liên kết Thư Viện Pháp Luật**
...
Kỹ năng `ccba-tvpl-vip-crawler` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Liên kết Thư Viện Pháp Luật ]
          │
          ▼
    >>> [ ccba-tvpl-vip-crawler ] <<<  (Thu thập nội dung số hóa bản quyền từ hệ thống TVPL VIP.)
          │
          ▼
[ ccba-legal-ingest ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Liên kết Thư Viện Pháp Luật`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `ccba-legal-ingest`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-tvpl-vip-crawler/SKILL.md --enforce-gpi
```
