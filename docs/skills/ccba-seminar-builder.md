...
> **Mô tả ngắn**: Chuẩn bị nội dung seminar/training nội bộ CCBA. Tạo recap, agenda, outline, và archive nội dung các buổi thảo luận.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-seminar-builder
```
...
### Mục Đích Hoạt Động
Chuẩn bị nội dung seminar/training nội bộ CCBA. Tạo recap, agenda, outline, và archive nội dung các buổi thảo luận.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Chuẩn bị đề cương, recap và lưu trữ tài liệu các buổi hội thảo nội bộ.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Nội dung đào tạo hoặc chia sẻ nội bộ CCBA**
...
Kỹ năng `ccba-seminar-builder` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Nội dung đào tạo hoặc chia sẻ nội bộ CCBA ]
          │
          ▼
    >>> [ ccba-seminar-builder ] <<<  (Chuẩn bị đề cương, recap và lưu trữ tài liệu các buổi hội thảo nội bộ.)
          │
          ▼
[ Bộ tài liệu seminar trong .md/teach/ ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Nội dung đào tạo hoặc chia sẻ nội bộ CCBA`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Bộ tài liệu seminar trong .md/teach/`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-seminar-builder/SKILL.md --enforce-gpi
```
