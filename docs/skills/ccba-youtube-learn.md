...
> **Mô tả ngắn**: Khảo cổ học Niềm tin (Belief Archaeology) thông qua bóc tách phụ đề và hình ảnh slide học thuật từ các video YouTube/bài giảng.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-youtube-learn
```
...
### Mục Đích Hoạt Động
Khảo cổ học Niềm tin (Belief Archaeology) thông qua bóc tách phụ đề và hình ảnh slide học thuật từ các video YouTube/bài giảng.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Trích xuất phụ đề, phân tích nội dung và tổng hợp tri thức từ video.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Video YouTube bài giảng / Hội thảo công nghệ**
...
Kỹ năng `ccba-youtube-learn` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Video YouTube bài giảng / Hội thảo công nghệ ]
          │
          ▼
    >>> [ ccba-youtube-learn ] <<<  (Trích xuất phụ đề, phân tích nội dung và tổng hợp tri thức từ video.)
          │
          ▼
[ Bản tóm tắt tri thức cốt lõi ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Video YouTube bài giảng / Hội thảo công nghệ`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Bản tóm tắt tri thức cốt lõi`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-youtube-learn/SKILL.md --enforce-gpi
```
