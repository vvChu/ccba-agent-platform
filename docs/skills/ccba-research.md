...
> **Mô tả ngắn**: Nghiên cứu chuyên sâu một vấn đề kỹ thuật hoặc pháp lý đối chiếu với các nguồn tài liệu gốc đáng tin cậy bằng cách khởi chạy subagent chạy ngầm (hỗ trợ Dual-Agent Adversarial).
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-research
```
...
| **Điểm Đánh Giá GPI (ADR-0057)** | `S=4.0 | K=3.0 | A=1.0 | P=1.0 (Tổng: 9.0)` |
...
### Mục Đích Hoạt Động
Nghiên cứu chuyên sâu một vấn đề kỹ thuật hoặc pháp lý đối chiếu với các nguồn tài liệu gốc đáng tin cậy bằng cách khởi chạy subagent chạy ngầm (hỗ trợ Dual-Agent Adversarial).
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Thực hiện khảo sát công nghệ, phân tích ưu nhược điểm và đề xuất giải pháp.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Vấn đề kỹ thuật cần nghiên cứu chuyên sâu**
...
Kỹ năng `ccba-research` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Vấn đề kỹ thuật cần nghiên cứu chuyên sâu ]
          │
          ▼
    >>> [ ccba-research ] <<<  (Thực hiện khảo sát công nghệ, phân tích ưu nhược điểm và đề xuất giải pháp.)
          │
          ▼
[ Tài liệu nghiên cứu kỹ thuật ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Vấn đề kỹ thuật cần nghiên cứu chuyên sâu`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Tài liệu nghiên cứu kỹ thuật`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-research/SKILL.md --enforce-gpi
```
