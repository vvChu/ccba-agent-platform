...
> **Mô tả ngắn**: Phát triển hướng kiểm thử (Red-Green-Refactor) giúp tạo mã nguồn ổn định, tin cậy thông qua các giao diện công khai (seams).
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-tdd
```
...
### Mục Đích Hoạt Động
Phát triển hướng kiểm thử (Red-Green-Refactor) giúp tạo mã nguồn ổn định, tin cậy thông qua các giao diện công khai (seams).
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Quy trình phát triển dẫn dắt bởi kiểm thử (Red - Green - Refactor).
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **ccba-to-spec / Ticket cần độ tin cậy cao**
...
Kỹ năng `ccba-tdd` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ ccba-to-spec / Ticket cần độ tin cậy cao ]
          │
          ▼
    >>> [ ccba-tdd ] <<<  (Quy trình phát triển dẫn dắt bởi kiểm thử (Red - Green - Refactor).)
          │
          ▼
[ ccba-implement / ccba-code-review ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `ccba-to-spec / Ticket cần độ tin cậy cao`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `ccba-implement / ccba-code-review`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-tdd/SKILL.md --enforce-gpi
```
