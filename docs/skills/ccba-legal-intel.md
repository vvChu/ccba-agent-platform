...
> **Mô tả ngắn**: Autonomous legal intelligence agent to crawl, diff, and generate compliance checklists from Vietnamese legal documents.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-legal-intel
```
...
### Mục Đích Hoạt Động
Autonomous legal intelligence agent to crawl, diff, and generate compliance checklists from Vietnamese legal documents.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Tự động thu thập, phân tích khác biệt (diff) và tạo compliance checklist.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Văn bản quy phạm pháp luật mới ban hành**
...
Kỹ năng `ccba-legal-intel` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Văn bản quy phạm pháp luật mới ban hành ]
          │
          ▼
    >>> [ ccba-legal-intel ] <<<  (Tự động thu thập, phân tích khác biệt (diff) và tạo compliance checklist.)
          │
          ▼
[ ccba-completion-checklist / Bảng kiểm tra ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Văn bản quy phạm pháp luật mới ban hành`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `ccba-completion-checklist / Bảng kiểm tra`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-legal-intel/SKILL.md --enforce-gpi
```
