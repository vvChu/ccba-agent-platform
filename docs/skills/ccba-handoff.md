...
> **Mô tả ngắn**: Đóng gói và tổng hợp phiên làm việc hiện tại thành tài liệu Handoff chuẩn mực để Agent tiếp theo tiếp quản liền mạch.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-handoff
```
...
### Mục Đích Hoạt Động
Đóng gói và tổng hợp phiên làm việc hiện tại thành tài liệu Handoff chuẩn mực để Agent tiếp theo tiếp quản liền mạch.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Đóng gói bối cảnh, bài học kinh nghiệm và bàn giao phiên sạch sẽ.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Kết thúc phiên làm việc / Sắp cạn Context Budget**
...
Kỹ năng `ccba-handoff` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Kết thúc phiên làm việc / Sắp cạn Context Budget ]
          │
          ▼
    >>> [ ccba-handoff ] <<<  (Đóng gói bối cảnh, bài học kinh nghiệm và bàn giao phiên sạch sẽ.)
          │
          ▼
[ Phiên làm việc sạch tiếp theo của Agent ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Kết thúc phiên làm việc / Sắp cạn Context Budget`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Phiên làm việc sạch tiếp theo của Agent`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-handoff/SKILL.md --enforce-gpi
```
