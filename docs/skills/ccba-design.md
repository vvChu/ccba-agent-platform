...
> **Mô tả ngắn**: Design brand identity, logos, banners, and visual assets. Use for brand systems, design tokens, corporate identity programs. Not for UI code patterns.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-design
```
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Thiết kế kiến trúc hệ thống và giao diện người dùng mạch lạc.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Yêu cầu giao diện hoặc trải nghiệm người dùng mới**
...
Kỹ năng `ccba-design` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Yêu cầu giao diện hoặc trải nghiệm người dùng mới ]
          │
          ▼
    >>> [ ccba-design ] <<<  (Thiết kế kiến trúc hệ thống và giao diện người dùng mạch lạc.)
          │
          ▼
[ ccba-to-spec / ccba-implement ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Yêu cầu giao diện hoặc trải nghiệm người dùng mới`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `ccba-to-spec / ccba-implement`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-design/SKILL.md --enforce-gpi
```
