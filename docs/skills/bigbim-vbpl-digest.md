...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Tóm lược các nghị định, thông tư và tiêu chuẩn ISO 19650 cho BIM.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Yêu cầu tra cứu quy chuẩn BIM Việt Nam**
...
Kỹ năng `bigbim-vbpl-digest` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Yêu cầu tra cứu quy chuẩn BIM Việt Nam ]
          │
          ▼
    >>> [ bigbim-vbpl-digest ] <<<  (Tóm lược các nghị định, thông tư và tiêu chuẩn ISO 19650 cho BIM.)
          │
          ▼
[ Tư vấn thiết kế / Thẩm tra tuân thủ ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Yêu cầu tra cứu quy chuẩn BIM Việt Nam`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `Tư vấn thiết kế / Thẩm tra tuân thủ`.
...
python scripts/validate_skills.py --file .agents/skills/bigbim-vbpl-digest/SKILL.md --enforce-gpi
```
