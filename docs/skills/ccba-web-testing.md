...
> **Mô tả ngắn**: Web testing with Playwright, Vitest, k6. E2E, load, visual, and a11y testing. Use for test automation, flakiness, Core Web Vitals, and cross-browser.
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-web-testing
```
...
### Mục Đích Hoạt Động
Web testing with Playwright, Vitest, k6. E2E, load, visual, and a11y testing. Use for test automation, flakiness, Core Web Vitals, and cross-browser.
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Kiểm thử tự động giao diện End-to-End và kiểm chứng hành vi người dùng.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Giao diện web / REST API đã hoàn thiện**
...
Kỹ năng `ccba-web-testing` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Giao diện web / REST API đã hoàn thiện ]
          │
          ▼
    >>> [ ccba-web-testing ] <<<  (Kiểm thử tự động giao diện End-to-End và kiểm chứng hành vi người dùng.)
          │
          ▼
[ ccba-code-review / Báo cáo nghiệm thu ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Giao diện web / REST API đã hoàn thiện`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `ccba-code-review / Báo cáo nghiệm thu`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-web-testing/SKILL.md --enforce-gpi
```
