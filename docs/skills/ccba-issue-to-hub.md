...
> **Mô tả ngắn**: Soạn thảo và gửi đề xuất ý tưởng/tính năng/báo lỗi (RFC Proposal) từ Spoke lên Hub dưới dạng GitHub Issue
...
```bash
python scripts/spoke/sync_spoke.py --skills ccba-issue-to-hub
```
...
### Mục Đích Hoạt Động
Soạn thảo và gửi đề xuất ý tưởng/tính năng/báo lỗi (RFC Proposal) từ Spoke lên Hub dưới dạng GitHub Issue
...
- Khi cần thực thi nghiệp vụ liên quan trực tiếp đến vai trò: Tiếp nhận, lọc trùng lặp và chuyển giao vấn đề lên Hub để xử lý.
- Trong chuỗi phát triển khi nhận tín hiệu bàn giao từ: **Sự cố hoặc yêu cầu phát sinh từ Spoke**
...
Kỹ năng `ccba-issue-to-hub` giữ vị trí then chốt trong chuỗi giá trị tích hợp của nền tảng:

```text
[ Sự cố hoặc yêu cầu phát sinh từ Spoke ]
          │
          ▼
    >>> [ ccba-issue-to-hub ] <<<  (Tiếp nhận, lọc trùng lặp và chuyển giao vấn đề lên Hub để xử lý.)
          │
          ▼
[ GitHub Issue đã được phân loại trên Hub ]
```

- **Đầu vào (Upstream)**: Nhận bối cảnh từ `Sự cố hoặc yêu cầu phát sinh từ Spoke`.
- **Thực thi (In-flight)**: Áp dụng các quy tắc kỹ thuật và công cụ tự động hóa để sản sinh kết quả chuẩn mực.
- **Đầu ra & Bàn giao (Downstream)**: Chuyển giao thành phẩm sạch sẽ sang `GitHub Issue đã được phân loại trên Hub`.
...
python scripts/validate_skills.py --file .agents/skills/ccba-issue-to-hub/SKILL.md --enforce-gpi
```
