...
def worker(req_id: int) -> dict:
    ...
    try:
        ...
        return {"id": req_id, "status": "success", "latency": latency, "response": res_snippet}
    except Exception as e:
        latency = time.time() - start_time
        return {"id": req_id, "status": "error", "latency": latency, "error": str(e)}
...
def main():
    ...
    if not os.environ.get("AI_GATEWAY_KEY") and not os.environ.get("OPENAI_API_KEY"):
        print(
            "⚠️ WRN: Chưa thấy biến môi trường 'AI_GATEWAY_KEY' hoặc 'OPENAI_API_KEY'. System sẽ dùng API Key mặc định nếu có trong .env",
            file=sys.stderr,
        )

    print("=" * 60)
    print("🚀 BẮT ĐẦU STRESS-TEST AI GATEWAY (LITELLM LOAD BALANCING)")
    print("🎯 Model mục tiêu: reasoning-gemma (Alias)")
        ...
        for future in concurrent.futures.as_completed(future_to_req):
            ...
            try:
                ...
                if res["status"] == "success":
                    print(
                        f"[Req {req_id:02d}] ✅ OK  | Latency: {res['latency']:^5.2f}s | {res['response']}"
                    )
                else:
                    print(
                        f"[Req {req_id:02d}] ❌ ERR | Latency: {res['latency']:^5.2f}s | Lỗi: {res['error']}"
                    )
