...
*   `[CALIBRATION] [Tinh Chỉnh Khép Kín Bộ Evaluator v2/v3 Trước Giờ G 00:00]`: Khắc phục hiện tượng sập Điểm Liệt giả định ở `BimClassificationScorer`, nối mạch Progressive Disclosure cho mock PCCC trong `tuner.py`, ánh xạ dataset `eval_bigbim_classification_redteam.json`, và bổ sung bộ lọc frontmatter `auto-tune: false` bảo vệ reference skills (Commit [`bd7a14c1`](https://github.com/vvChu/ccba-agent-platform/commit/bd7a14c1)). Đạt 94.3% BIM, 100% PCCC, 0 Điểm Liệt.
