...
- [x] **Chặn đẩy nhánh rỗng (Empty Push Guard):**
...
- [x] **Khóa đơn nhiệm (Process Mutex / Flock):**
...
- [x] **Cách ly HEAD sạch giữa 2 Daemons:**
...
- [x] **Đổi đối soát sang `origin/main`:**
...
- [x] **Mở rộng phạm vi dọn dẹp:**
  - Quét cả các nhánh tiền tố `docs/auto-refactor-*` bên cạnh `auto-tune/nightly-*`.
- [x] **Dọn dẹp cả remote branch rỗng (nếu có PAT hỗ trợ):**
...
1. [x] Thử nghiệm chạy liên tiếp 2 tiến trình `run_nightly_tuner.sh` song song: tiến trình thứ 2 lập tức dừng lại qua `flock` mà không gây tranh chấp index lock (`⚠️ [LOCK BUSY] Một tiến trình Nightly Tuner khác đang chạy. Dừng thực thi an toàn.`).
2. [x] Chạy `doc_refactor_daemon.py` khi không có tài liệu nào cần sửa: Không có nhánh rác mới nào được đẩy lên GitHub remote. Nhánh rỗng cục bộ tự động được thu hồi (`test_empty_push_guard_when_zero_commits` PASSED).
3. [x] Hàm `_cleanup_old_empty_branches` dọn dẹp thành công các nhánh rác tồn đọng > 7 ngày đối soát với `origin/main` và cố gắng xóa nhánh remote (`test_cleanup_old_empty_branches_logic` PASSED).
4. [x] Unit tests `scripts/tests/test_nightly_tuner_daemon.py` và `scripts/tests/test_doc_refactor_daemon.py` đạt 100% PASS.
