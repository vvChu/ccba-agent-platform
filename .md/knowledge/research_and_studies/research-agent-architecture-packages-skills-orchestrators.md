...
* **Bản chất của con số >40% sự cố production:**
  ...
  - *Thực chứng nội bộ CCBA:* Trong lịch sử vận hành nền tảng, lỗi này đã xảy ra trực tiếp tại [ADR 0051 (Mục 2.3)](file:///C:/Users/chuvu/.gemini/antigravity/worktrees/ccba-agent-platform/migrate_ccba_to_skills/docs/adr/0051-hub-spoke-sync-hardening-constitution-preservation-and-virtual-fallback.md#L10), khi hàm `sync_legal_assets` trong package thay đổi chữ ký nhưng `sdk_inspector.py` vẫn truyền `target_spoke=...`, gây lỗi `TypeError: unexpected keyword argument` tại Spoke.
...
| :--- | :--- | :--- |
| **Zero-Cost Hard CI Gates** | Linter tĩnh kiểm tra cấu trúc metadata trước khi build/merge. | `skill_validator.py` ([packages/ccba-harness](file:///C:/Users/chuvu/.gemini/antigravity/worktrees/ccba-agent-platform/migrate_ccba_to_skills/packages/ccba-harness/src/ccba_harness/skill_validator.py)): Kiểm tra Frontmatter schema, cấm trùng tên, cảnh báo shallow skills (< 35 dòng). |
...
| **Hợp Nhất Bảo Toàn (Non-Destructive Merge)** | Phân tích cây cú pháp (AST) hoặc Markdown Headings để chỉ hợp nhất phần dùng chung. | `Non-Destructive Section Merge` trong [`sync_spoke.py`](file:///C:/Users/chuvu/.gemini/antigravity/worktrees/ccba-agent-platform/migrate_ccba_to_skills/scripts/sync_spoke.py): Giữ nguyên vẹn 100% các phần tùy biến nội bộ của Spoke (`🛡️ PRESERVED`). |
| **Bộ Dọn Rác & Ánh Xạ Chuyển Hướng (Tombstones)** | Đổi tên tệp cũ sang `.bak`, duy trì bảng tra alias để chuyển hướng tự động. | Bảng ánh xạ `SKILL_DEPRECATION_ALIASES` (107 entries trong [`coordinator.py`](file:///C:/Users/chuvu/.gemini/antigravity/worktrees/ccba-agent-platform/migrate_ccba_to_skills/scripts/spoke/sync/coordinator.py)) dọn dẹp sạch sẽ 100% Zombie Bloat tại Spoke. |
