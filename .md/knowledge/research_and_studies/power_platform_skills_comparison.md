...
| --- | --- | --- | --- |
| **1. Target Runtime** | Cấu hình cho GitHub Copilot CLI và Claude Code (giao diện `/plugin`). | Cấu hình cho Antigravity IDE (giao diện Custom Slash Commands `/ccba-*`). | **Khác biệt nền tảng**: Cần chuyển đổi tệp đặc tả XML/JSON của họ thành định nghĩa [SKILL.md](../../../.agents/skills/_core/idop-scaffolder/SKILL.md) chuẩn của Antigravity. |
