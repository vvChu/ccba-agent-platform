...
#### 94. Cross-Platform Safe Process Liveness Probe Pattern
- **Ngữ cảnh**: Kiểm tra trạng thái còn sống (liveness) của một tiến trình (PID) trong cơ chế khóa tệp (`FileMutexLock`, `TVPLSessionMutex`) hoặc daemon singleton runner.
- **Giải pháp**: **Tuyệt đối không dùng `os.kill(pid, 0)` trên Windows**. Trên Windows, CPython ánh xạ `os.kill(pid, 0)` sang `TerminateProcess(hProcess, 0)` — khiến việc kiểm tra PID của chính mình (`os.getpid()`) hoặc tiến trình khác sẽ **giết chết ngay lập tức tiến trình đó với exit code 0**. Thay vào đó, hãy sử dụng hàm đa nền tảng an toàn:
  ```python
  def _is_process_alive(pid: int | None) -> bool:
      ...
      if sys.platform.startswith("win"):
          try:
              ...
              is_active = (kernel32.GetExitCodeProcess(handle, ctypes.byref(exit_code)) and exit_code.value == 259)
          ...
          try:
              ...
              return False
  ```
- **Nguồn**: Session `0e98ff1e-51c5-4e15-8e0f-d4b44c15ae5e`, 2026-08-14

#### 95. Mocking Seam Alignment in Deep Module Migrations
- **Ngữ cảnh**: Di chuyển mã nguồn từ các script rời rạc (như `scripts/legal/legal_sync.py`) vào thư viện lõi (`ccba_legal.sync`).
- **Giải pháp**: Phải rà soát và cập nhật đồng bộ các unit tests đang mock `sys.modules["scripts.legal_sync"]` sang mock trực tiếp vào module chính quy (`patch("ccba_legal.sync.GOOGLE_API_AVAILABLE", True)`). Nếu không, code nghiệp vụ mới sẽ đọc biến thật từ package (mặc định `False`) và rơi vào nhánh fallback chạy thật dẫn đến timeout 30s.
- **Nguồn**: Session `0e98ff1e-51c5-4e15-8e0f-d4b44c15ae5e`, 2026-08-14
...
#### 58. Using `os.kill(pid, 0)` for Process Liveness on Windows
- **Vấn đề**: Nhầm tưởng `os.kill(pid, 0)` trên Windows là vô hại như POSIX. Thực chất Windows CPython gọi `TerminateProcess` khiến tiến trình test runner / agent host tự kết liễu đột ngột với exit code 0 mà không để lại traceback.
- **Thay thế bằng**: Sử dụng `OpenProcess` + `GetExitCodeProcess` với cờ `STILL_ACTIVE = 259`.

#### 59. Superficial "User Cancelled" Attribution
- **Vấn đề**: Khi IDE hiển thị thông báo "User cancelled agent execution", vội vàng kết luận là người dùng bấm nút hủy và thử chạy lại một cách mù quáng. Thực chất, đây là triệu chứng của việc tiến trình con bị đứt kết nối socket/IPC bất ngờ (do crash, tự sát bằng TerminateProcess, hoặc Timeout Watchdog).
- **Thay thế bằng**: Dùng quy trình `/diagnosing-bugs` thu hẹp chính xác test case và dòng code nào kích hoạt sự cố ngắt tiến trình.
