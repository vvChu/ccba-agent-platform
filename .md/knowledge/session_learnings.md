...
- **RULE-1.1 [ADR 0057 — Khung 2 Giai Đoạn & Chỉ Số GPI]**:
  - Cổng 0: Deep Seams (`packages/*/src/`). `SKILL.md` cấm code logic trần. Cổng 1: Tier 3 Composite Orchestrator.
...
- **RULE-1.14 [ADR 0050 & ADR 0051 — Hub-Mediated Spoke Discovery & Virtual-First Retrieval]**:
  - Spoke chỉ cần `hub_path`. SDK tự tra `spoke_registry_decrypted.yaml` trên Hub định vị Spoke pháp điển (`ccba-legal-knowledge`).
  - Phân giải 3 tầng: T1 (CLI `get-clause`/`.\.md\legal_docs\`) $\rightarrow$ T2 (Master Registry) $\rightarrow$ T3 (SSOT `legal_registry.yaml`). Cấm copy cả kho luật về Spoke (tránh nghẽn OneDrive, MAX_PATH).
...
- **RULE-2.2 [Spoke CI Gates Verification Pipeline]**: 5 Cổng: (1) `lint_visual_parity`, (2) `validate_legal_spoke`, (3) `test_converter_regression`, (4) `verify_all_docs_against_pdf` (SHA-256), (5) `verify_cross_links`.
...
- **RULE-2.12 [Deterministic Completion, Fail-Fast Security & Socket Safety]**:
  ...
  - Fail-Fast Maskara: Chặn API keys đầu `query_rag()`. Socket: `is_port_open` gán `timeout <= 1.0s` chống treo Windows.
...
- **RULE-4.1 [Entry Point Duy Nhất Khi Có Issue ID: `/ccba-new-feature`]**: Có Issue ID, LUÔN đề xuất `/ccba-new-feature #<id>` (8 bước Factory Model). CẤM làm tắt.
...
- **RULE-4.11 [OpenAI SDK Embedding & Reasoning Timeout Scaling]**: `embed()` Gemini: `extra_body={"drop_params": True}` (`gemini-embedding-2`). Reasoning: sàn `max_tokens` $\ge 16,384$, auto-timeout `max(timeout, max_tokens/50.0)`.
