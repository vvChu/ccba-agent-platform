...
- **RULE-1.7 [ADR 0046 — Sanitized Fleet Telemetry]**: Trích xuất số phi định danh (`tokens`, `cost`). Cấm log prompt/dữ liệu khách hàng.
...
- **RULE-1.14 [ADR 0050/0051 — Spoke Discovery & Virtual Retrieval]**: Spoke chỉ cần `hub_path`, tra `spoke_registry_decrypted.yaml`. 3 tầng: T1 (CLI `get-clause`) $\rightarrow$ T2 (Registry) $\rightarrow$ T3 (SSOT `legal_registry.yaml`). Cấm copy kho.
...
- **RULE-2.7 [Cross-Platform Safe-Remove, Junctions & POSIX +x]**: `safe_remove`: Win junction (`0x400`) dùng `rmdir`, cấm đệ quy link target. `_make_writable`: Giữ `+x` (`st_mode | S_IWUSR`), cấm mode tĩnh `0o600` trên POSIX dirs.
...
- **RULE-2.12 [Deterministic Completion, Fail-Fast Security & Socket Safety]**: Hard Completion (ADR-0058): CLI `sync` khi 0 bundle hoặc fallback cloud BẮT BUỘC exit 1. Maskara: chặn keys đầu `query_rag()`. Socket clamp `timeout <= 1.0s` trên Windows.
- **RULE-2.13 [Tuner Circuit Breaker & Compaction]**: `LLMTaskAdapter` default `CircuitBreaker()`. Caller ghi đè env `CCBA_TUNER_TOKEN_BUDGET`. Prompt compaction (> 300 dòng) strip comments và bullets `- Cập nhật quy chuẩn...`.
- **RULE-2.14 [Gate 1b Ruff Format Check]**: Python BẮT BUỘC chạy `ruff check .` và `ruff format --check .` trước khi push (Gate 1b chặn trên 3.10/3.11/3.12).

---

## Miền 3. 📜 Chuẩn Mực Pháp Lý & Dữ Liệu (Legal & Data Standards)

- **RULE-3.1 [Hiệu Lực Pháp Lý Tuyệt Đối — Từ 01/07/2026]**: Mọi VB viện dẫn BẮT BUỘC CURRENT: **Luật XD 2025** (`135/2025/QH15`), **NĐ 217/2026/NĐ-CP** (thay NĐ 175 & 15), **NĐ 207/2026/NĐ-CP** (thay NĐ 06). CẤM VB hết hiệu lực.
...
- **RULE-3.4 [ADR 0059 — Nguyên Văn 100% & Chống Bịa Đặt Dữ Liệu Pháp Lý]**: CẤM bịa đặt VBPL; trích dẫn nguyên văn. Thiếu tệp: dùng `TVPLCrawler` tải PDF/DOCX chính thức, đóng dấu SHA-256 (`pdf_sha256`), kiểm `validate_bundle_provenance()`.
- **RULE-3.5 [Deep Seam CLI Retrieval Over Raw File Ingestion]**: Dùng `python -m ccba_legal get-clause --doc <id> --clause <id>` (< 500 tokens) thay vì `view_file` file MD thô (~60k tokens).
...
- **RULE-4.5 [AI Gateway Spark Auth & Fast-Inference Gating]**: LiteLLM Spark: Bearer `${SPARK_API_KEY}` (qua env). Ưu tiên `gemini-3.7-flash` (< 1s), route `qwen-local-primary`.
- **RULE-4.6 [Tier 3 Orchestrator — ADR-0057 / ADR-0058]**: SSOT tại `.agents/skills/ccba-platform/SKILL.md`, Single-Writer. Spoke sync gọi `ccba-harness verify-patch`.
...
- **RULE-4.9 [RSA-OAEP & Telemetry Heartbeat — ADR-0046]**: RSA-2048 OAEP plaintext $\le 190$ bytes. Tách metadata tĩnh khỏi telemetry (`.md/telemetry/spoke_heartbeats.yaml`).
...
- **RULE-4.11 [Embedding & Reasoning Timeout Scaling]**: `embed()`: `extra_body={"drop_params": True}` (`gemini-embedding-2`). Reasoning: `max_tokens` $\ge 16,384$, auto-timeout `max(timeout, max_tokens/50.0)`.
...
- **RULE-4.13 [Cross-Shell PR Body]**: `gh pr create`: Truyền body qua env (`--body "$PR_BODY"`). CẤM escape `\n` trong quotes/PowerShell.
- **RULE-4.14 [Merge Danger Triage]**: PR review & planning phân loại: Door (`Two-way` vs `One-way`) và Blast Radius (`Localized` / `Package-wide` / `Monorepo-wide` / `Spoke-affecting`).
- **RULE-4.15 [Self-Healing PR CLI & Scoped Tuner]**: `gh pr create` fallback retry không nhãn khi lỗi. Tuner daemon & runner hỗ trợ `--skill <name>` chạy kiểm thử mẻ nhỏ.
...
- **RULE-5.2 [Windows Path Quotes & Hook Protection]**: Windows: IDE bọc `hooks.json` trong `"C:\..."` $\rightarrow$ vô hiệu bằng `{}` và khóa `IsReadOnly = $true` trên PowerShell.
