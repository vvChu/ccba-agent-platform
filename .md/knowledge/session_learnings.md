...
- **RULE-1.14 [ADR 0050/0051 — Spoke Discovery & Virtual-First Retrieval]**: Spoke chỉ cần `hub_path`. Tra `spoke_registry_decrypted.yaml` tìm legal corpus. 3 tầng: T1 (CLI `get-clause`/`.\.md\legal_docs\`) $\rightarrow$ T2 (Master Registry) $\rightarrow$ T3 (SSOT `legal_registry.yaml`). Cấm copy cả kho về Spoke.
...
- **RULE-2.7 [Cross-Platform Safe-Remove, Junctions & POSIX +x]**: `safe_remove`: Win junction (`0x400`) dùng `rmdir`, cấm đệ quy/chmod link target. `_make_writable`: Giữ `+x` (`st_mode | S_IWUSR`), cấm mode tĩnh `0o600` làm mất `S_IXUSR` trên POSIX dirs.
...
- **RULE-2.12 [Deterministic Completion, Fail-Fast Security & Socket Safety]**: Hard Completion (ADR-0058): CLI `sync` khi 0 bundle hoặc fallback cloud BẮT BUỘC exit 1. Maskara: chặn keys đầu `query_rag()`. Socket `is_port_open` clamp `timeout <= 1.0s` trên Windows.
...
- **RULE-3.1 [Hiệu Lực Pháp Lý Tuyệt Đối — Từ 01/07/2026]**: Mọi VB viện dẫn BẮT BUỘC CURRENT. Chặn LLM bias. HIỆN HÀNH: **Luật XD 2025** (`135/2025/QH15`), **NĐ 217/2026/NĐ-CP** (thay NĐ 175 & 15), **NĐ 207/2026/NĐ-CP** (thay NĐ 06). CẤM VB hết hiệu lực.
...
- **RULE-5.5 [Worktree Isolation & Git Safety]**: Worktree: `cd "$WORKTREE_DIR"` trước export `PYTHONPATH`. Dọn nhánh: check `diff_res.returncode == 0` trước khi duyệt stdout rỗng.
