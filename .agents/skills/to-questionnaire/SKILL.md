...
1. **Track 1: Platform Track (Spoke ➔ Hub RFCs)**:
   ...
   - *Hành động:* Tự động đóng gói các câu hỏi thành bản đề xuất cải tiến chuẩn CCBA RFC và tích hợp trực tiếp với workflow [`/ccba-issue-to-hub`](../workflows/ccba-issue-to-hub.md) để mở GitHub Issue lên repository trung tâm (`ccba-agent-platform`).
...
4. **Bàn giao quy trình tiếp theo (Workflow Hand-off):**
   - Kích hoạt kỹ năng [`/ccba-to-spec`](../skills/to-spec/SKILL.md) để chuyển hóa quyết định thành PRD / Đặc tả kỹ thuật.
   - Kích hoạt [`/ccba-to-tickets`](../skills/to-tickets/SKILL.md) để phân rã nhiệm vụ phát triển.
