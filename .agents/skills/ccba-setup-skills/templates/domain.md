...
- **`docs/adr/`** (hoặc `.md/knowledge/adr/`) — Đọc các tài liệu Quyết định Kiến trúc (ADRs) liên quan đến phần tính năng chuẩn bị code. Đối với monorepo/multi-context, kiểm tra thêm thư mục ADRs cục bộ của từng module con (ví dụ: `src/<context>/docs/adr/`).
...
├── .md/knowledge/
│   └── adr/                      ← Chứa các quyết định kiến trúc
│       ├── 0001-setup-db.md
│       └── 0002-use-jwt.md
└── src/
```
